/* AuthContext holds my app's global authentication state (user, token, login/logout, etc. )
useEffect - triggers effects, like fetching user data, on dependency changes
useState - manages internal state */
import { createContext, useEffect, useState } from "react";

/* creates a context object to share auth data across components 
- AuthContext is used by components via useContext(AuthContext) to access auth 
  data and methods */
export const AuthContext = createContext();

// defines a context provider component, and children is any JSX nested inside AuthProvider
export function AuthProvider({ children }) {
  ...
  -- passes a function that returns the value to useState() to delay execution of an 
     expensive operation until the component's initial render, function here will only 
     run once, and subsequent renders uses the stored state value */
